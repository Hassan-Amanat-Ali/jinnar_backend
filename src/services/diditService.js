...
class DiditService {
  ...
  verifySignature(rawBody, signature, secret, timestamp) {
    if (!signature || !secret) {
      console.warn(
        "[Didit Debug] Missing signature or secret for verification",
      );
      return false;
    }
    ...
    if (timestamp) {
      ...
      console.log(
        `[Didit Debug] Timestamp Check - Sent: ${sentAt.toISOString()}, Now: ${now.toISOString()}, Diff: ${diff}ms`,
      );
  ...
  parseVerificationStatus(event) {
    // Didit sends 'Approved', 'Declined', 'In Review', 'In Progress', 'Not Started'
  ...
  /**
   * Fetches the full session decision (including document metadata) from Didit
   * @param {string} sessionId
   * @returns {Object}
   */
  async getSessionDecision(sessionId) {
    try {
      const url = `https://verification.didit.me/v3/session/${sessionId}/decision/`;
      const apiKey = process.env.DIDIT_SECRET_KEY;

      if (!apiKey) {
        throw new Error("Missing Didit API Key");
      }
      ...
      console.error(
        `Didit Get Session Decision Error [${sessionId}]:`,
        error.response?.data || error.message,
      );
      throw new Error(
        `Failed to fetch verification results for session ${sessionId}`,
      );
    }
  }
}
