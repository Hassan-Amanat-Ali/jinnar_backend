...
export default function verifyWebhook(options = {}) {
  ...
  const secret =
    ...
    process.env.PAWAPAY_CALLBACK_SECRET;
  ...
  return async function (req, res, next) {
    try {
      ...
      const plainSecret =
        req.headers[plainSecretHeader] ||
        req.headers[plainSecretHeader.toLowerCase()];
      ...
      if (!verified) {
        return res
          .status(401)
          .json({ error: "Invalid webhook signature or secret" });
      }
