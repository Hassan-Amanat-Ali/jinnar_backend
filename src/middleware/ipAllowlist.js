...
export default function ipAllowlist(allowed = null) {
  ...
  const envList = process.env.PAWAPAY_ALLOWED_IPS || '3.64.89.224/32,110.39.11.3/32';
  const cidrs = Array.isArray(allowed) && allowed.length ? allowed : envList.split(',').map(s => s.trim()).filter(Boolean);
