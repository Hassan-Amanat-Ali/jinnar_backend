...
const getIdentifierType = (input) => {
  ...
  if (phoneRegex.test(cleanPhone) || /^\+\d{7,15}$/.test(cleanPhone))
    return "mobileNumber";
...
const sendTwilioOtp = async (number) => {
  if (!twilioClient) {
    console.log(
      `[DEV ONLY] Twilio disabled. OTP for ${number} would be sent here.`,
    );
...
const checkTwilioOtp = async (number, code) => {
  if (!twilioClient) {
    console.log(
      `[DEV ONLY] Twilio disabled. Checking OTP for ${number} with code ${code}.`,
    );
  ...
  try {
    ...
    console.log(
      `Twilio Verify check for ${number} status: ${verification_check.status}`,
    );
...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (!identifier || !role || !password) {
      return res
        .status(400)
        .json({ error: "Email/Phone, role, and password are required" });
    }
    ...
    if (savedUser && savedUser._id) {
      console.log(
        `[Rollback] Deleting user ${savedUser._id} due to messaging failure.`,
      );
    ...
    return res.status(500).json({
      error:
        "Registration failed. Please check your phone number/email format.",
...
export const verifyCode = async (req, res, next) => {
  try {
    ...
    if (!identifier || !code)
      return res
        .status(400)
        .json({ error: "Identifier (Email/Phone) and code are required" });
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    if (!identifier || !code || !newPassword) {
      return res
        .status(400)
        .json({ error: "Identifier, code, and new password are required." });
    }
...
export const initiateContactChange = async (req, res, next) => {
  try {
    ...
    if (conflictUser) {
      ...
      return res
        .status(409)
        .json({ error: "This contact is already in use by another account." });
    }
...
export const verifyContactChange = async (req, res, next) => {
  try {
    ...
    if (!user || !user.tempContact || !user.tempContact.value) {
      return res
        .status(400)
        .json({ error: "No pending contact change found." });
    }

    if (user.tempContact.expires < Date.now()) {
      return res
        .status(400)
        .json({ error: "Expired request. Please try again." });
    }
...
export const switchRole = async (req, res, next) => {
  try {
    ...
    if (!newRole || !["buyer", "seller"].includes(newRole)) {
      return res
        .status(400)
        .json({ error: "Invalid role. Provide 'buyer' or 'seller'." });
    }
    ...
    if (user.role === newRole) {
      return res
        .status(200)
        .json({ message: `Already in '${newRole}' role.`, role: user.role });
    }
